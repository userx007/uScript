...
============================================================================================ */
bool BuspiratePlugin::m_handle_onewire_reset( const char *pstrArgs ) const
{
    char request = 0x02;
    char answer  = 0x01;
...
============================================================================================ */
bool BuspiratePlugin::m_handle_onewire_search( const char *pstrArgs ) const
{
    bool bRetVal = true;
    unsigned char request = 0U;

    if      (0 == strcmp("rom",   pstrArgs)) { request = (unsigned char)0xF0U; }
    else if (0 == strcmp("alarm", pstrArgs)) { request = (unsigned char)0xECU; }
...
============================================================================================ */
bool BuspiratePlugin::m_handle_onewire_read( const char *pstrArgs ) const
{
    bool bRetVal = true;
...
============================================================================================ */
bool BuspiratePlugin::m_handle_onewire_write( const char *pstrArgs ) const
{
...
============================================================================================ */
bool BuspiratePlugin::m_handle_onewire_cfg( const char *pstrArgs ) const
{
    bool bRetVal = true;
    char request = 0x40;
    ...
    if (0 == strcmp("help", pstrArgs)) {
        ...
        // pin output
        if (NULL != strchr(pstrArgs, 'w') ) { BIT_CLEAR(request, 3); }
        if (NULL != strchr(pstrArgs, 'W') ) { BIT_SET(request,   3); }
        // clock idle phase
        if (NULL != strchr(pstrArgs, 'p') ) { BIT_CLEAR(request, 2); }
        if (NULL != strchr(pstrArgs, 'P') ) { BIT_SET(request,   2); }
        // clock edge
        if (NULL != strchr(pstrArgs, 'a') ) { BIT_CLEAR(request, 1); }
        if (NULL != strchr(pstrArgs, 'A') ) { BIT_SET(request,   1); }
        // sample time
        if (NULL != strchr(pstrArgs, 'c') ) { BIT_CLEAR(request, 0); }
        if (NULL != strchr(pstrArgs, 'C') ) { BIT_SET(request,   0); }
