    ...
    std::string_view txId, std::string_view rxId) const
{
    ...
    while (sent < data.size()) {
        ...
        if (segResp[0] == kSdoAbort) {
            result.status = ICommDriver::Status::NACK;
            return result;
        }
        if ((segResp[0] >> 5) != kScsDlSegment) {
            result.status = ICommDriver::Status::PROTOCOL_ERROR;
            return result;
        }
        if (((segResp[0] >> 4) & 1) != toggle) {
            result.status = ICommDriver::Status::PROTOCOL_ERROR;
            return result;
        }
    ...
    std::string_view txId, std::string_view rxId) const
{
    ...
    if (blksize == 0) {
        blksize = m_cfg.canOpenBlockSize;
    }
    if (blksize == 0) {
        blksize = 1;
    }
    ...
    while (sent < data.size()) {
        ...
        if (ackResp[0] == kSdoAbort) {
            result.status = ICommDriver::Status::NACK;
            return result;
        }
        if ((ackResp[0] >> 5) != kScsBlock || (ackResp[0] & 0x03) != 2) {
            result.status = ICommDriver::Status::PROTOCOL_ERROR;
            return result;
        }
    ...
    if (endResp[0] == kSdoAbort) {
        result.status = ICommDriver::Status::NACK;
        return result;
    }
    if ((endResp[0] >> 5) != kScsBlock || (endResp[0] & 0x03) != 1) {
        result.status = ICommDriver::Status::PROTOCOL_ERROR;
        return result;
    }
    ...
    std::string_view rxId, std::string_view txId) const
{
    ...
    if (!sInd) {
        result.status = ICommDriver::Status::PROTOCOL_ERROR;
        return result;
    } // size required, see class comment
    ...
    if ((endResp[0] >> 5) != kScsBlockUl || (endResp[0] & 0x03) != 1) {
        result.status = ICommDriver::Status::PROTOCOL_ERROR;
        return result;
    }
