...
namespace {
    ...
    ICommDriver::Status libusb_err_to_status(int iRc)
    {
        switch (iRc) {
        case LIBUSB_SUCCESS:
    ...
    inline void put_u32le(uint8_t *pu8P, uint32_t u32V)
    {
        pu8P[0] = static_cast<uint8_t>(u32V & 0xFF);
        pu8P[1] = static_cast<uint8_t>((u32V >> 8) & 0xFF);
        pu8P[2] = static_cast<uint8_t>((u32V >> 16) & 0xFF);
        pu8P[3] = static_cast<uint8_t>((u32V >> 24) & 0xFF);
    }

    inline uint32_t get_u32le(const uint8_t *pu8P)
    {
        return static_cast<uint32_t>(pu8P[0]) | (static_cast<uint32_t>(pu8P[1]) << 8) | (static_cast<uint32_t>(pu8P[2]) << 16) | (static_cast<uint32_t>(pu8P[3]) << 24);
    }
    ...
    void serialize_u32_struct(const uint32_t *pu32Fields, size_t count, uint8_t *pu8Out)
    {
        for (size_t i = 0; i < count; ++i) {
            put_u32le(pu8Out + i * 4, pu32Fields[i]);
        }
    }

    void deserialize_u32_struct(const uint8_t *pu8In, uint32_t *pu32Fields, size_t count)
    {
        for (size_t i = 0; i < count; ++i) {
            pu32Fields[i] = get_u32le(pu8In + i * 4);
        }
    }
...
Candlelight::Candlelight(uint16_t u16Vendor_id, uint16_t u16Product_id, unsigned device_index,
                         const std::string &strIdentityLabel)
    : m_strIdentityLabel(strIdentityLabel)
{
    open(u16Vendor_id, u16Product_id, device_index);
}
...
ICommDriver::Status Candlelight::open(uint16_t u16Vendor_id, uint16_t u16Product_id, unsigned device_index)
{
    close();
    ...
    for (ssize_t i = 0; i < count; ++i) {
        ...
        if (desc.idVendor == u16Vendor_id && desc.idProduct == u16Product_id) {
...
ICommDriver::Status Candlelight::ctrl_out(GsUsbBreq eReq, uint16_t u16Value, const void *pvData, uint16_t u16Len, uint32_t u32Timeout_ms)
{
    if (!is_open()) {
        return Status::PORT_ACCESS;
    }
    ...
    int rc = libusb_control_transfer(m_usbHandle, USB_DIR_OUT_VENDOR_IFACE,
                                     static_cast<uint8_t>(eReq), u16Value,
                                     static_cast<uint16_t>(m_interfaceNum),
                                     const_cast<unsigned char *>(static_cast<const unsigned char *>(pvData)),
                                     u16Len, u32Timeout_ms);
    if (rc < 0 || static_cast<uint16_t>(rc) != u16Len) {
        LOG_PRINT(LOG_WARNING, LOG_HDR; LOG_STRING("ctrl_out failed, breq="); LOG_UINT8(static_cast<uint8_t>(eReq)));
        return (rc == LIBUSB_ERROR_TIMEOUT) ? Status::WRITE_TIMEOUT : libusb_err_to_status(rc < 0 ? rc : LIBUSB_ERROR_IO);
    }
    return Status::SUCCESS;
}

ICommDriver::Status Candlelight::ctrl_in(GsUsbBreq eReq, uint16_t u16Value, void *pvData, uint16_t u16Len, uint32_t u32Timeout_ms)
{
    if (!is_open()) {
        return Status::PORT_ACCESS;
    }
    ...
    int rc = libusb_control_transfer(m_usbHandle, USB_DIR_IN_VENDOR_IFACE,
                                     static_cast<uint8_t>(eReq), u16Value,
                                     static_cast<uint16_t>(m_interfaceNum),
                                     static_cast<unsigned char *>(pvData),
                                     u16Len, u32Timeout_ms);
    ...
    if (rc < 0) {
        LOG_PRINT(LOG_WARNING, LOG_HDR; LOG_STRING("ctrl_in failed, breq="); LOG_UINT8(static_cast<uint8_t>(eReq)));
...
ICommDriver::Status Candlelight::set_bittiming(uint32_t u32Prop_seg, uint32_t u32Phase_seg1, uint32_t u32Phase_seg2,
                                               uint32_t u32Sjw, uint32_t u32Brp, uint32_t u32Timeout_ms)
{
    if (m_channel_open) {
        LOG_PRINT(LOG_WARNING, LOG_HDR; LOG_STRING("set_bittiming called with channel open"));
        return Status::INVALID_PARAM;
    }
    uint32_t fields[5] = {u32Prop_seg, u32Phase_seg1, u32Phase_seg2, u32Sjw, u32Brp};
    uint8_t buf[20];
    serialize_u32_struct(fields, 5, buf);
    return ctrl_out(GsUsbBreq::BITTIMING, 0, buf, sizeof(buf), u32Timeout_ms);
}

ICommDriver::Status Candlelight::set_data_bittiming(uint32_t u32Prop_seg, uint32_t u32Phase_seg1, uint32_t u32Phase_seg2,
                                                    uint32_t u32Sjw, uint32_t u32Brp, uint32_t u32Timeout_ms)
{
    if (m_channel_open) {
    ...
    uint8_t buf[20];
    serialize_u32_struct(fields, 5, buf);
    return ctrl_out(GsUsbBreq::DATA_BITTIMING, 0, buf, sizeof(buf), u32Timeout_ms);
}
...
namespace {
     ...
     */
    bool calc_bittiming(uint32_t u32Fclk_can, uint32_t u32Bitrate_bps, double dSample_point,
                        uint32_t u32Tseg1_min, uint32_t u32Tseg1_max,
                        uint32_t u32Tseg2_min, uint32_t u32Tseg2_max,
                        uint32_t u32Sjw_max, uint32_t u32Brp_min, uint32_t u32Brp_max, uint32_t u32Brp_inc,
                        uint32_t &u32Out_prop_seg, uint32_t &u32Out_phase_seg1, uint32_t &u32Out_phase_seg2,
                        uint32_t &u32Out_sjw, uint32_t &u32Out_brp)
    {
        if (u32Bitrate_bps == 0 || u32Fclk_can == 0) {
            return false;
        }
        ...
        for (uint32_t brp = u32Brp_min; brp <= u32Brp_max; brp += (u32Brp_inc ? u32Brp_inc : 1)) {
            ...
            const uint64_t denom = static_cast<uint64_t>(brp) * u32Bitrate_bps;
            if (denom == 0) {
                continue;
            }
            if (u32Fclk_can % denom != 0) {
                continue; // must divide exactly - no bit-rate error tolerated
            }
            const uint64_t tqTotal = u32Fclk_can / denom;
            if (tqTotal < 3) {
            ...
            for (uint32_t tseg2 = u32Tseg2_min; tseg2 <= u32Tseg2_max && tseg2 < segTotal; ++tseg2) {
                const uint32_t tseg1 = segTotal - tseg2;
                if (tseg1 < u32Tseg1_min || tseg1 > u32Tseg1_max) {
                    continue;
                }

                const double samplePoint = static_cast<double>(1 + tseg1) / static_cast<double>(tqTotal);
                const double err         = std::abs(samplePoint - dSample_point);
                if (err < bestErr) {
                    bestErr   = err;
                    bestBrp   = brp;
                    bestTseg1 = tseg1;
        ...
        u32Out_prop_seg   = bestTseg1 / 2;
        u32Out_phase_seg1 = bestTseg1 - u32Out_prop_seg;
        u32Out_phase_seg2 = bestTseg2;
        u32Out_sjw        = std::min(u32Sjw_max, u32Out_phase_seg2);
        u32Out_brp        = bestBrp;
        return true;
    }
...
ICommDriver::Status Candlelight::set_bitrate(uint32_t u32Bitrate_bps, double dSample_point, uint32_t u32Timeout_ms)
{
    uint32_t prop_seg = 0, phase_seg1 = 0, phase_seg2 = 0, sjw = 0, brp = 0;
    if (!calc_bittiming(m_btConst.fclk_can, u32Bitrate_bps, dSample_point,
    ...
    return set_bittiming(prop_seg, phase_seg1, phase_seg2, sjw, brp, u32Timeout_ms);
}

ICommDriver::Status Candlelight::set_fd_data_bitrate(uint32_t u32Bitrate_bps, double dSample_point, uint32_t u32Timeout_ms)
{
    if (!is_fd_supported()) {
    ...
    uint32_t prop_seg = 0, phase_seg1 = 0, phase_seg2 = 0, sjw = 0, brp = 0;
    if (!calc_bittiming(m_btConstExt.fclk_can, u32Bitrate_bps, dSample_point,
    ...
    return set_data_bittiming(prop_seg, phase_seg1, phase_seg2, sjw, brp, u32Timeout_ms);
}
...
ICommDriver::Status Candlelight::open_channel(uint32_t u32Mode_flags, uint32_t u32Timeout_ms)
{
    uint32_t fields[2] = {static_cast<uint32_t>(GsCanMode::START), u32Mode_flags};
    uint8_t buf[8];
    serialize_u32_struct(fields, 2, buf);

    Status s = ctrl_out(GsUsbBreq::MODE, 0, buf, sizeof(buf), u32Timeout_ms);
    if (s == Status::SUCCESS) {
        m_channel_open  = true;
        m_fd_negotiated = (u32Mode_flags & GS_CAN_MODE_FD) != 0;
        LOG_PRINT(LOG_DEBUG, LOG_HDR; LOG_STRING("CAN channel opened, fd="); LOG_UINT8(m_fd_negotiated ? 1 : 0));
    }
    return s;
}

ICommDriver::Status Candlelight::close_channel(uint32_t u32Timeout_ms)
{
...
ICommDriver::Status Candlelight::get_state(GsDeviceState &sState, uint32_t u32Timeout_ms)
{
    ...
    uint8_t buf[12] = {0};
    Status s        = ctrl_in(GsUsbBreq::GET_STATE, 0, buf, sizeof(buf), u32Timeout_ms);
    if (s != Status::SUCCESS) {
        return s;
    }
...
size_t Candlelight::encode_frame(uint32_t u32Echo_id, const CanFrame &sFrame, std::span<uint8_t> out) const
{
    if (out.size() < max_packet_len()) {
        return 0;
    }
    if (sFrame.is_canfd && !m_fd_negotiated) {
        return 0;
    }

    const uint8_t dataLen = sFrame.is_canfd ? sFrame.len : (sFrame.is_remote ? 0 : std::min<uint8_t>(sFrame.len, 8));

    uint32_t can_id       = sFrame.id;
    ...
    if (!sFrame.is_remote) {
        std::copy(sFrame.data.begin(), sFrame.data.begin() + dataLen, p + GS_HOST_FRAME_HDR_LEN);
    }
...
bool Candlelight::decode_frame(const uint8_t *pu8Pkt, size_t len, uint32_t &u32Echo_id, CanFrame &sFrame) const
{
    if (!pu8Pkt || len < GS_HOST_FRAME_HDR_LEN) {
        return false;
    }
    ...
    sFrame.is_extended = (can_id_raw & GS_CAN_EFF_FLAG) != 0;
    sFrame.is_remote   = (can_id_raw & GS_CAN_RTR_FLAG) != 0;
    sFrame.is_canfd    = is_fd;
    sFrame.brs         = is_fd && (flags & GS_CAN_FLAG_BRS) != 0;
    sFrame.id          = sFrame.is_extended ? (can_id_raw & GS_CAN_EFF_MASK) : (can_id_raw & GS_CAN_SFF_MASK);
    sFrame.dlc         = can_dlc;
    sFrame.len         = is_fd ? ucanframe::dlc_to_len(can_dlc) : std::min<uint8_t>(can_dlc, 8);

    sFrame.data.fill(0);
    if (!sFrame.is_remote) {
        std::copy(pu8Pkt + GS_HOST_FRAME_HDR_LEN, pu8Pkt + GS_HOST_FRAME_HDR_LEN + sFrame.len, sFrame.data.begin());
    }
...
ICommDriver::Status Candlelight::bulk_write_frame(uint32_t u32Echo_id, const CanFrame &sFrame, uint32_t u32Timeout_ms)
{
    std::vector<uint8_t> buf(max_packet_len());
    size_t n = encode_frame(u32Echo_id, sFrame, std::span<uint8_t>(buf));
    if (n == 0) {
        return Status::INVALID_PARAM;
    }

    int transferred = 0;
    int rc          = libusb_bulk_transfer(m_usbHandle, m_epOut, buf.data(), static_cast<int>(n), &transferred, u32Timeout_ms);
...
ICommDriver::Status Candlelight::bulk_read_one(uint32_t &u32Echo_id, CanFrame &sFrame, uint32_t u32Timeout_ms, std::stop_token stop_tok)
{
    std::vector<uint8_t> buf(max_packet_len());
    ...
    const bool bInfinite            = (u32Timeout_ms == 0);
    ...
    if (!decode_frame(buf.data(), static_cast<size_t>(transferred), u32Echo_id, sFrame)) {
...
ICommDriver::Status Candlelight::send_frame(const CanFrame &sFrame, uint32_t u32Timeout_ms, std::stop_token stop_tok)
{
    if (!m_channel_open) {
    ...
    Status s = bulk_write_frame(echoId, sFrame, u32Timeout_ms);
    if (s != Status::SUCCESS) {
        return s;
    }
...
ICommDriver::Status Candlelight::receive_frame(CanFrame &sFrame, uint32_t u32Timeout_ms, std::stop_token stop_tok)
{
    ...
    for (;;) {
        ...
        uint32_t echoId = 0;
        Status s        = bulk_read_one(echoId, sFrame, remainingMs, stop_tok);
        if (s != Status::SUCCESS) {
