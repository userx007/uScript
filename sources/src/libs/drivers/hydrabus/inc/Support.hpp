...
namespace HydraHAL {
     ...
     */
    inline uint8_t set_bit(uint8_t u8Byte_val, int iBit, int iPosition)
    {
        if (iBit == 1) {
     ...
     */
    inline uint32_t from_le32(const std::vector<uint8_t> &vBuf, size_t offset = 0)
    {
        return static_cast<uint32_t>(vBuf[offset]) | static_cast<uint32_t>(vBuf[offset + 1]) << 8 | static_cast<uint32_t>(vBuf[offset + 2]) << 16 | static_cast<uint32_t>(vBuf[offset + 3]) << 24;
    }

    /**
     * @brief Decode a 4-byte big-endian buffer to uint32_t.
     */
    inline uint32_t from_be32(const std::vector<uint8_t> &vBuf, size_t offset = 0)
    {
        return static_cast<uint32_t>(vBuf[offset]) << 24 | static_cast<uint32_t>(vBuf[offset + 1]) << 16 | static_cast<uint32_t>(vBuf[offset + 2]) << 8 | static_cast<uint32_t>(vBuf[offset + 3]);
    }
