/*
 * FT4232H async UART driver — Linux platform implementation
 *
 * Channels C and D of the FT4232H are async UART-only (no MPSSE engine).
 * Opened with libftdi1 using INTERFACE_C or INTERFACE_D.
 * ftdi_set_bitmode is NOT called — leaves channel in async serial mode.
 *
                                           ...
                                           uint8_t u8DeviceIndex)
{
    ...
    // Map Channel::C → INTERFACE_C (3), Channel::D → INTERFACE_D (4)
    ftdi_interface iface = (channel == FT4232Base::Channel::C) ? INTERFACE_C : INTERFACE_D;
    ...
    if (ftdi_usb_open_desc_index(ctx,
                                 static_cast<int>(FT4232Base::FT4232H_VID),
                                 static_cast<int>(FT4232Base::FT4232H_PID),
                                 nullptr, nullptr,
...
FT4232UART::Status FT4232UART::close()
{
    ...
    LOG_PRINT(LOG_VERBOSE, LOG_HDR; LOG_STRING("FT4232H UART closed"));
    return Status::SUCCESS;
}
