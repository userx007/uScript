...
|   vxlapi_linux.h
| Project:
|   Vector XL Driver Library - Linux port
|
| Description:
|   Unofficial Linux adaptation of Vector's vxlapi.h (Windows), reworked to
|   declare/compile cleanly with GCC/Clang on Linux and to be binary
|   compatible with the supplied libXlApi_so_26_20.14 (i.e. libXlApi.so.26.20.14).
|
|   This file is derived from the original vxlapi.h shipped by Vector
|   Informatik GmbH for the Windows XL Driver Library. All bus-specific
|   constants, struct layouts, enums and comments are kept verbatim; only
|   the Windows-only plumbing (calling convention, DLL export macros,
|   HANDLE type, and the __int64/long typedefs) has been changed, listed
|   below. See the accompanying README for details, the full list of
|   symbols actually implemented in libXlApi_so_26_20.14, and a build
|   example.
|
|   Changes vs. the original Windows vxlapi.h:
|     1. __stdcall / __declspec(dllexport) removed - x86-64 Linux (SysV ABI)
|        has a single calling convention, and this header is used purely
|        for *declaring* prototypes against a pre-built .so, not for
|        exporting symbols from it.
|     2. `#include <pshpackN.h>` / `<poppack.h>` replaced with the
|        equivalent `#pragma pack(push, N)` / `#pragma pack(pop)`, which
|        GCC and Clang support with identical semantics to MSVC. This
|        preserves the exact on-the-wire struct layouts (e.g. XL_CAN_EV_*,
|        driver config structs) required by the library.
|     3. `unsigned __int64` / `__int64` -> `uint64_t` / `int64_t`
|        (<stdint.h>). Same 8-byte size on both platforms, so this is a
|        pure spelling fix, not an ABI change.
|     4. `HANDLE` (XLhandle) -> `int`. Windows uses a kernel HANDLE (8
|        bytes) for xlSetNotification[Handle]; the Linux build uses an
|        eventfd file descriptor instead (confirmed by disassembling the
|        supplied .so - xlCoreSetNotification takes an `int*`). Wait for
|        readiness with select()/poll()/epoll() on this fd instead of
|        WaitForSingleObject().
|     5. `typedef long XLlong; typedef unsigned long XLulong;` ->
|        `typedef int32_t XLlong; typedef uint32_t XLulong;`
|        *** THIS IS THE IMPORTANT ONE ***
|        On Windows (LLP64), `long` is 4 bytes, so XLportHandle (= XLlong)
|        and friends are 4 bytes. On native 64-bit Linux (LP64), a bare
|        `long` is 8 bytes. Disassembly of the supplied library shows every
|        XLportHandle/XLulong parameter is still passed/read as a 32-bit
|        value (`edi`, `edx`, 32-bit register moves), i.e. Vector kept the
|        4-byte width for their Linux build too. Leaving this as `long`
|        would silently corrupt every call's argument layout on Linux.
|
|   None of the struct field types elsewhere in this header needed
|   changing: everything but the two typedefs above already uses
|   platform-width-stable types (char/short/int/unsigned variants),
|   which are the same size under both the Windows and Linux x86-64 ABIs.
|
...
/* Linux: no DLL export/calling-convention concept - both macros become no-ops. */
...
// supplied libXlApi_so_26_20.14 - do NOT change to `long`/`unsigned long`,
// which are 8 bytes on 64-bit Linux and would break every call using
...
#define XL_INVALID_DEVICE_INDEX  (unsigned int)0xFFFFFFFF
    ...
    // Linux: this is an eventfd file descriptor, not a Windows HANDLE. Pass it
    // to select()/poll()/epoll_wait() to block until the driver signals it,
    // then read() the eventfd (or just re-arm) instead of WaitForSingleObject().
...
#define XL_TS_DOMAIN_STAT_DOMAIN_IN_SYNC 0x04U
    ...
    typedef enum e_xl_timesync_interface_version {
        XL_TS_INTERFACE_VERSION_INVL = 0,
        XL_TS_INTERFACE_VERSION_1    = 1
    } XLtsInterfaceVersion;
