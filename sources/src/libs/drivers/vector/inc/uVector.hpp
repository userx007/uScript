...
class Vector : public ICommDriver {
    ...
    public:
        ...
        static constexpr uint8_t canFdLenToDlc(size_t szLen)
        {
            for (uint8_t i = 0; i < FD_DLC_LENGTH_TABLE.size(); ++i) {
                if (FD_DLC_LENGTH_TABLE[i] >= szLen) {
                    return i;
                }
            }
        ...
        XLaccess getAccessMask() const
        {
            return m_xlAccessMask;
        }
        ...
        void setFdDataBitrate(uint32_t u32DataBitrate)
        {
            m_u32FdDataBitrate = u32DataBitrate;
        }

        uint32_t getFdDataBitrate() const
        {
            return m_u32FdDataBitrate;
        }

        /** true (default) = ISO 11898-1:2015 CAN FD, false = pre-standard Bosch/"non-ISO" CAN FD. */
        void setFdIso(bool bIso)
        {
            m_bFdIso = bIso;
        }
        ...
        void setFdPaddingByte(uint8_t u8Byte)
        {
            m_u8FdPaddingByte = u8Byte;
        }

        uint8_t getFdPaddingByte() const
        {
            return m_u8FdPaddingByte;
        }
    ...
    private:
        ...
        class RawIo final : public ICommDriver {
            public:
                explicit RawIo(const Vector &owner)
                    : m_owner(owner)
                {
                }
