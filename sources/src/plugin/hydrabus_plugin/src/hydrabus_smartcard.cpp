...
{
    if (args == "help") {
        LOG_PRINT(LOG_FIXED, LOG_HDR; LOG_STRING("Use: rst [0|1]"));
        return true;
    }
    auto* p = m_smartcard();
    if (!p) return false;
...
{
    if (args == "help") {
        LOG_PRINT(LOG_FIXED, LOG_HDR; LOG_STRING("Use: baud N"));
        return true;
    }
    auto* p = m_smartcard();
    if (!p) return false;
...
{
    if (args == "help") {
        LOG_PRINT(LOG_FIXED, LOG_HDR; LOG_STRING("Use: prescaler N  (0-255)"));
        return true;
    }
    auto* p = m_smartcard();
    if (!p) return false;
...
{
    if (args == "help") {
        LOG_PRINT(LOG_FIXED, LOG_HDR; LOG_STRING("Use: guardtime N  (0-255)"));
        return true;
    }
    auto* p = m_smartcard();
    if (!p) return false;
...
{
    if (args == "help") {
        LOG_PRINT(LOG_FIXED, LOG_HDR; LOG_STRING("Use: write AABB.."));
        return true;
    }
    auto* p = m_smartcard();
    if (!p) return false;
...
{
    if (args == "help") {
        LOG_PRINT(LOG_FIXED, LOG_HDR; LOG_STRING("Retrieve card ATR"));
        return true;
    }
    auto* p = m_smartcard();
    if (!p) return false;
    ...
    auto atr = p->get_atr();
    LOG_PRINT(LOG_FIXED, LOG_HDR; LOG_STRING("ATR:"));
