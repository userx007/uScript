...
bool CP2112Plugin::m_CP2112_INFO(const std::string& args, std::stop_token st ) const
{
    ...
    LOG_PRINT(LOG_EMPTY, LOG_STRING("Args   : [x=DEVICE_INDEX] [c=I2C_CLOCK] [a=I2C_ADDRESS] [r=READ_TIMEOUT] [sd=SCRIPT_DELAY]"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("Usage  : CP2112.CONFIG c=400000 a=0x51"));
    LOG_SEP();
...
{
    return generic_module_dispatch<CP2112Plugin>(this, "I2C", args, st);
}

bool CP2112Plugin::m_CP2112_GPIO(const std::string& args, std::stop_token st ) const
{
    return generic_module_dispatch<CP2112Plugin>(this, "GPIO", args, st);
}
