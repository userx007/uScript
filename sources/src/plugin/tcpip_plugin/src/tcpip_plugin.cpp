...
bool TCPIPPlugin::m_TCPIP_INFO(const std::string& args, std::stop_token st) const
{
    ...
    LOG_PRINT(LOG_EMPTY, LOG_STRING("[TCPIP]"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("ARTEFACTS_PATH       =            # directory used by SCRIPT/CMD/wrrdf for reading/writing artefact files"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("TCP_HOST             = 127.0.0.1  # remote host to connect to"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("TCP_PORT             = 5000       # remote TCP port to connect to"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("TCP_CONNECT_TIMEOUT  = 3000       # connect timeout in ms"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("TCP_READ_TIMEOUT     = 2000       # read timeout in ms"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("TCP_WRITE_TIMEOUT    = 2000       # write timeout in ms"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("TCP_READ_BUFFER_SIZE = 1024       # size in bytes of the local read buffer"));
