/*
 * FT2232 Plugin – UART protocol handlers
 *
 * Uses FT2232UART driver on the FT2232D async serial channel (channel B).
 * FT2232H has no dedicated UART channel and is rejected at open() time.
 *
 * Subcommands:
 *   open   [baud=N] [data=8] [stop=1] [parity=none|odd|even|mark|space]
 *          [flow=none|hw] [variant=D] [device=N]
 *   close
...
bool FT2232Plugin::m_handle_uart_help(const std::string&) const
{
...
bool FT2232Plugin::m_handle_uart_open(const std::string& args) const
{
    ...
    if (m_sUartCfg.variant != FT2232Base::Variant::FT2232D) {
        LOG_PRINT(LOG_ERROR, LOG_HDR;
                  LOG_STRING("UART is only supported on FT2232D (channel B). Use variant=D"));
        return false;
    }
...
bool FT2232Plugin::m_handle_uart_close(const std::string&) const
{
    if (!m_pUART) {
...
bool FT2232Plugin::m_handle_uart_cfg(const std::string& args) const
{
    if (args == "help") {
        LOG_PRINT(LOG_EMPTY,
...
bool FT2232Plugin::m_handle_uart_write(const std::string& args) const
{
    if (args == "help") {
    ...
    auto r = pDrv->tout_write(pDrv->FT2232_UART_WRITE_DEFAULT_TIMEOUT, data);
...
bool FT2232Plugin::m_handle_uart_read(const std::string& args) const
{
    if (args == "help") {
...
bool FT2232Plugin::m_handle_uart_script(const std::string& args) const
{
    if (args == "help") {
