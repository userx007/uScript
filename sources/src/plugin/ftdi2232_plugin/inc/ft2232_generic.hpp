...
#define FT2_WRITE_MAX_CHUNK_SIZE  ((size_t)(4096U))
#define FT2_BULK_MAX_BYTES        ((size_t)(65536U))  // MPSSE max per transfer
                              ...
                              const std::string& args)
{
    ...
    if (it != pMap->end()) return (pOwner->*it->second)(args);
                              ...
                              const std::string& args)
{
    ...
    if (!pOwner->isEnabled()) return true;
...
bool generic_write_read_data(const T* pOwner, const std::string& args, WrRdCbk<T> cbk)
{
    ...
    if (args[0] == ':') {
        ...
        if (parts.size() == 2 && !numeric::str2sizet(parts[1], readLen)) return false;
    }
                              ...
                              const std::string& artefactsPath)
{
    ...
    size_t wrChunk = FT2_WRITE_MAX_CHUNK_SIZE;
    size_t rdChunk = FT2_WRITE_MAX_CHUNK_SIZE;
    ...
    if (wrChunk == 0) wrChunk = FT2_WRITE_MAX_CHUNK_SIZE;
    if (rdChunk == 0) rdChunk = FT2_WRITE_MAX_CHUNK_SIZE;
