...
extern "C"
{
    EXPORTED DigisparkPlugin* pluginEntry()
    {
        return new DigisparkPlugin();
    }

    EXPORTED void pluginExit(DigisparkPlugin *ptrPlugin)
    {
...
bool DigisparkPlugin::doInit(void * /*pvUserData*/)
{
    m_bIsInitialized = true;
    return m_bIsInitialized;
}

void DigisparkPlugin::doCleanup(void)
{
...
bool DigisparkPlugin::m_DIGISPARK_INFO(const std::string& args) const
{
    ...
    LOG_SEP();
    LOG_PRINT(LOG_EMPTY, LOG_STRING(DIGISPARK_PLUGIN_NAME); LOG_STRING("Vers:"); LOG_STRING(m_strVersion));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("Build:"); LOG_STRING(__DATE__); LOG_STRING(__TIME__));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("Description: Digispark ATtiny85 USB→I2C and USB→SPI master bridge"));
    LOG_SEP();
    ...
    LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage : DIGISPARK.CONFIG v:0x16C0 p:0x05DF r:2000 w:2000"));
    LOG_SEP();
    LOG_PRINT(LOG_EMPTY, LOG_STRING("I2C_SCAN  : scan all 7-bit I2C addresses (1-126)"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("  Args    : (none)"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage   : DIGISPARK.I2C_SCAN"));
    LOG_SEP();
    ...
    LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage   : DIGISPARK.I2C_WRITE 3C 00 AF"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("          : DIGISPARK.I2C_WRITE 0x68 6B 00"));
    LOG_SEP();
    ...
    LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage   : DIGISPARK.I2C_READ 68 1"));
    LOG_SEP();
    ...
    LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage   : DIGISPARK.I2C_WRRD 68 75 1   (MPU-6050 WHO_AM_I)"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("          : DIGISPARK.I2C_WRRD 68 3B 5   (MPU-6050 accel X+Y)"));
    LOG_SEP();
    ...
    LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage   : DIGISPARK.SPI_CFG m:0 d:1"));
    LOG_SEP();
    ...
    LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage   : DIGISPARK.SPI_WRITE DE AD BE EF"));
    LOG_SEP();
    ...
    LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage   : DIGISPARK.SPI_READ 4"));
    LOG_SEP();
    LOG_PRINT(LOG_EMPTY, LOG_STRING("SPI_XFER  : full-duplex transfer, print MISO  (max 6 bytes)"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("  Args    : <byte0_hex> [<byte1_hex> ...]"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage   : DIGISPARK.SPI_XFER 9F 00 00   (JEDEC ID)"));
    LOG_SEP();
    ...
    LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage   : DIGISPARK.SPI_WRREG E0 B6   (BME280 reset)"));
    LOG_SEP();
    ...
    LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage   : DIGISPARK.SPI_RDREG D0 1   (BME280 chip-ID)"));
    LOG_SEP();
    ...
    LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage   : DIGISPARK.SCRIPT i2c_init.txt"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("          : DIGISPARK.SCRIPT spi_test.txt 50"));
    LOG_SEP();
 ...
 *
 * Usage:
 *   DIGISPARK.I2C_WRITE 3C 00 AF       (SSD1306: cmd byte + display-ON)
 ...
 *
 * Usage:
 *   DIGISPARK.I2C_READ 68 1    (read 1 byte from 0x68)
 ...
 *
 * Usage:
 *   DIGISPARK.I2C_WRRD 68 75 1    (MPU-6050 WHO_AM_I → expects 0x68)
 *   DIGISPARK.I2C_WRRD 68 3B 5    (MPU-6050 accel X/Y first 5 bytes)
 ...
 *
 * Usage:
 *   DIGISPARK.SPI_XFER 9F 00 00    (JEDEC ID — 3 bytes out, 3 bytes in)
 ...
 *
 * Usage:
 *   DIGISPARK.SPI_WRREG E0 B6    (BME280 soft-reset)
 ...
 *
 * Usage:
 *   DIGISPARK.SPI_RDREG D0 1    (BME280 chip-ID → expect 0x60)
...
/*----------------------------------------------------------------------------*/
/**
 * @brief SCRIPT — execute a sequence of DIGISPARK commands from a file.
 *
...
bool DigisparkPlugin::m_DIGISPARK_SCRIPT(const std::string& args) const
{
    ...
    for (const auto& strLine : vLines)
    {
        ...
        // Accept both  "DIGISPARK.CMD args"  and bare  "CMD args"
        std::string strCmd, strCmdArgs;
        const std::string kPrefix = std::string(DIGISPARK_PLUGIN_NAME) + ".";

        if (strTrimmed.rfind(kPrefix, 0) == 0)
        {
            // Full form: DIGISPARK.CMD args
...
/*----------------------------------------------------------------------------*/
bool DigisparkPlugin::m_LocalSetParams(const PluginDataSet *psSetParams)
{
    bool bRetVal = false;
...
bool DigisparkPlugin::m_I2CWrite(uint8_t                   u8Addr,
...
bool DigisparkPlugin::m_I2CRead(uint8_t                   u8Addr,
                                 size_t                    szLen,
...
bool DigisparkPlugin::m_I2CWriteRead(uint8_t                   u8Addr,
...
bool DigisparkPlugin::m_SPIWrite(std::span<const uint8_t>   data,
...
bool DigisparkPlugin::m_SPIRead(size_t                    szLen,
                                 std::span<uint8_t>         buffer,
                                 std::shared_ptr<SPIBridge>  shpDrv) const
{
...
bool DigisparkPlugin::m_SPITransfer(std::span<const uint8_t>   mosi,
...
bool DigisparkPlugin::m_ParseHexByte(const std::string& str, uint8_t& out)
{
    if (str.empty())
        return false;
...
bool DigisparkPlugin::m_ParseHexBytes(const std::vector<std::string>& tokens,
                                       size_t                          szStart,
...
bool DigisparkPlugin::m_ParseUint16(const std::string& str, uint16_t& out)
{
    if (str.empty())
        return false;
