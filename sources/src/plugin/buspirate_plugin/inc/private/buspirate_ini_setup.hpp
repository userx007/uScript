...
/*--------------------------------------------------------------------------------------------------------*/
/**
  * \brief Parse a key=value token stream and dispatch each token to the matching BUSPIRATE setter.
  *
  * Recognised keys:
  *   p    –  strUartPort          (calls setUartPort)
  *   b    –  u32UartBaudrate      (calls setUartBaudrate)
  *   r    –  u32ReadTimeout       (calls setReadTimeout)
  *   w    –  u32WriteTimeout      (calls setWriteTimeout)
  *   s    –  u32ReadBufferSize    (calls setReadBufferSize)
...
bool parseAndCallHandlers(const T *pOwner, const std::string& input)
{
    ...
    // "p" call void setter — handled as special case below.
