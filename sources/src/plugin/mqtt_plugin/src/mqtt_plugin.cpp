...
void MqttPlugin::doCleanup(void)
{
    ...
    if (m_pCommandDriver) {
        m_pCommandDriver->disconnect();
    }
    m_pCommandDriver.reset();
    if (m_pTcpip) {
        m_pTcpip->close();
    }
    m_pTcpip.reset();
...
// -----------------------------------------------------------------------
// Driver factory — see mqtt_plugin.hpp's class doc comment / m_OpenDriver()
// -----------------------------------------------------------------------

std::shared_ptr<MqttCommandDriver> MqttPlugin::m_OpenDriver(void) const
{
    if (m_pCommandDriver && m_pCommandDriver->is_open()) {
        return m_pCommandDriver;
    }
    ...
    // The real driver — see mqtt_command_driver.hpp's class doc comment:
    // TCPIP is used exactly as-is, unmodified.
    ...
    m_pCommandDriver = std::make_shared<MqttCommandDriver>(m_pTcpip, cfg, m_strHost);
    m_pCommandDriver->includeTopicOnReceive = m_bReceiveIncludeTopic;

    if (!m_pCommandDriver->ensureSession()) {
        LOG_PRINT(LOG_ERROR, LOG_HDR; LOG_STRING("MQTT session establishment failed"));
        m_pCommandDriver.reset();
        m_pTcpip.reset();
        return nullptr;
    }
...
bool MqttPlugin::m_MQTT_INFO(const std::string& args, std::stop_token st) const
{
    ...
    LOG_PRINT(LOG_EMPTY, LOG_STRING("Architecture: MqttProtocol (protocol) / TCPIP (real driver) / MqttCommandDriver (decorator) / this plugin"));
    LOG_SEP();
...
// -----------------------------------------------------------------------
// MQTT.CMD / MQTT.SCRIPT — standard pattern, see class doc comment
// -----------------------------------------------------------------------
...
bool MqttPlugin::m_MQTT_CMD(const std::string& args, std::stop_token st) const
{
    ...
    return ucmdexec::generic_cmd(
        args, m_bIsEnabled,
        [this]() -> std::shared_ptr<MqttCommandDriver> { return m_OpenDriver(); },
        MQTT_PLUGIN_NAME,
