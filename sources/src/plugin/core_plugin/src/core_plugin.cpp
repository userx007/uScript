...
bool CorePlugin::m_Utils_INFO (const std::string &args) const
{
    ...
    do {
        ...
        LOG_PRINT(LOG_EMPTY, LOG_STRING(CORE_PLUGIN_NAME); LOG_STRING("Vers:"); LOG_STRING(m_strPluginVersion));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("Build:"); LOG_STRING(__DATE__); LOG_STRING(__TIME__));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("Description: helper commands"));

        LOG_PRINT(LOG_EMPTY, LOG_STRING("--------------------"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("BREAKPOINT : stop execution and wait for user decision (any key=continue, Esc=abort)"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Args : [message]"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage: CORE.BREAKPOINT"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       CORE.BREAKPOINT Switch Power OFF then press a key"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("--------------------"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("DELAY : introduce a delay (milliseconds) in script execution"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Args : delay_ms"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage: CORE.DELAY 2000"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("--------------------"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("EVAL_VECT : evaluate two vectors against a rule; auto-detects type (number/version/string/bool)"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Args : op1 rule op2  (op: vector of values or $MACRONAME)"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       numeric/version rules: < <= == != >= >"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       string rules case sensitive: EQ NE"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       string rules case insensitive: eq ne"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage: RESULT ?= CORE.EVAL_VECT \"1 2 3 4\" == \"1 2 3 4\""));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       RESULT ?= CORE.EVAL_VECT $MACRO1 != $MACRO2"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       RESULT ?= CORE.EVAL_VECT $MACRO1 EQ \"hello\""));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Return : TRUE or FALSE (as string)"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("--------------------"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("EVAL_BOARRAY : evaluate an array of boolean values with AND/OR reduction"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Args : op1 op2 .. opN | rule"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       op: TRUE FALSE !TRUE !FALSE 0 1 !0 !1 or $MACRONAME"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       rule: AND OR"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage: RESULT ?= CORE.EVAL_BOARRAY TRUE !TRUE FALSE !FALSE 0 1 !1 !0 | AND"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       RESULT ?= CORE.EVAL_BOARRAY $M1 $M2 $M3 | OR"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Return : TRUE or FALSE (as string)"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("--------------------"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("EVAL_BOEXPR : evaluate a boolean expression with operators && || ! and parentheses"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Args : boolean expression"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage: RESULT ?= CORE.EVAL_BOEXPR (TRUE || FALSE) && !FALSE"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       RESULT ?= CORE.EVAL_BOEXPR $M1 && ($M2 || $M3)"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Return : TRUE or FALSE (as string)"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("--------------------"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("FAIL : force the script to fail, always or conditionally"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Args : [| condition]"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage: CORE.FAIL"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       CORE.FAIL | TRUE"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       CORE.FAIL | $MACRONAME"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Note : condition can be TRUE FALSE !TRUE !FALSE 0 1 !0 !1 $MACRONAME"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("--------------------"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("FORMAT : build a string by substituting items from an input vector using a pattern"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Args : input_string | format_pattern"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       format_pattern: literal text with %N replacing item at 0-based index N"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage: $RETVAL ?= CORE.FORMAT \"AA BB CC DD\" | \"%0:%1:%2:%3\""));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       results in: AA:BB:CC:DD"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       $RETVAL ?= CORE.FORMAT \"11 22 33 44\" | \"0x%0 0x%1\" == \"0x%2 0x%3\""));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       results in: 0x11 0x22 == 0x33 0x44"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Return : a string with the result"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("--------------------"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("MATH : perform math operation between 2 vectors of numbers or $MACRONAMES"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Args : op1 rule op2 [| HEX]"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       rule: + - * / % & | ^ << >> += -= *= /= %= &= |= ^= <<= >>="));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       | HEX : optional, output result in hexadecimal"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage: RESULT ?= CORE.MATH \"1 2 3\" + \"5 6 7\""));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       results in: 6 8 10"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       RESULT ?= CORE.MATH \"10 20\" + \"6 4\" | HEX"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       results in: 0x10 0x18"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("--------------------"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("MESSAGE : print a message (unconditional; use PRINT for conditional output)"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Args : message"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage: CORE.MESSAGE Please switch Power ON"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("--------------------"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("PRINT : print a message or macro value, optionally conditioned"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Args : message [| condition]"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage: CORE.PRINT $RETVAL"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       CORE.PRINT This is the message"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       CORE.PRINT This is the message | TRUE"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       CORE.PRINT $MACRONAME  | !FALSE"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       CORE.PRINT $MACRONAME1 | $MACRONAME2"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Note : condition can be TRUE FALSE !TRUE !FALSE 0 1 !0 !1 $MACRONAME"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("--------------------"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("RETURN : write a value to the caller macro"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Args : string (use \"\" if it contains spaces)"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage: RETVAL ?= CORE.RETURN \"11 22 33\""));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       RETVAL ?= CORE.RETURN \"HELLO\""));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       RETVAL ?= CORE.RETURN HELLO"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("--------------------"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("VALIDATE : compare two values based on a rule; fails the script if condition is not met"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Args : item1 rule item2  (use EVAL_VECT to get TRUE/FALSE instead of failing)"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Usage: CORE.VALIDATE $VAL1 == $VAL2"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       CORE.VALIDATE $VERS1 > $VERS2"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       CORE.VALIDATE $STR1 EQ $STR2"));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("  Note : number/version rules: < <= == != > >="));
        LOG_PRINT(LOG_EMPTY, LOG_STRING("       string rules: EQ NE eq ne"));
