...
bool FT232HPlugin::m_FT232H_INFO(const std::string& args, std::stop_token st ) const
{
    ...
    LOG_PRINT(LOG_EMPTY, LOG_STRING("         as the ini file, see the [FT232H] section below)"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("Usage  : FT232H.CONFIG KEY1=value1 [KEY2=value2 ...]"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("         e.g. FT232H.CONFIG SPI_CLOCK=2000000 I2C_ADDRESS=0x51"));
    LOG_SEP();
    LOG_PRINT(LOG_EMPTY, LOG_STRING("INI file parameters (copy/paste into your ini file):"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("[FT232H]"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("ARTEFACTS_PATH =          # directory used by SCRIPT/CMD/wrrdf for reading/writing artefact files"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("DEVICE_INDEX   = 0        # index of the FT232H USB device to open, when more than one is attached"));
...
/**
  ...
  *        and m_FT232H_CONFIG() (runtime CONFIG command) -- single source of truth for which
...
PluginSettingsBinder FT232HPlugin::m_BuildSettingsBinder ( void ) const
{
...
/**
  ...
  *       for the [FT232H] ini section -- reuses the same binder/validation as the
  *       ini loader (m_BuildSettingsBinder()), so anything settable via the ini file is
  *       settable here too.
  *
  * \note Usage example: <br>
  *       FT232H.CONFIG SPI_CLOCK=2000000 I2C_ADDRESS=0x51
  *
