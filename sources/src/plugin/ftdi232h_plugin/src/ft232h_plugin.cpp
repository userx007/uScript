...
bool FT232HPlugin::m_FT232H_INFO(const std::string& args, std::stop_token st ) const
{
    ...
    LOG_PRINT(LOG_EMPTY, LOG_STRING("Args   : [x=DEVICE_INDEX] [spf=SPI_CLOCK] [i2f=I2C_CLOCK] [a=I2C_ADDRESS] [r=READ_TIMEOUT] [sd=SCRIPT_DELAY] [baud=UART_BAUD]"));
    LOG_PRINT(LOG_EMPTY, LOG_STRING("Usage  : FT232H.CONFIG spf=2000000 a=0x51"));
    LOG_SEP();
...
/**
  ...
  *       FT232H.CONFIG spf=2000000 a=0x51
  *
  * \param[in] args space-separated key=value tokens (see inc/private/ft232h_setup.hpp)
  *
