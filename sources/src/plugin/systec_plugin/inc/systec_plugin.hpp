...
#define SYSTEC_PLUGIN_VERSION    "1.0.0.1"
#define SYSTEC_PLUGIN_NAME       "SYSTEC"

/////////////////////////////////////////////////////////////////////////////////
//                          PLUGIN MACROS                                      //
/////////////////////////////////////////////////////////////////////////////////

// SYSTEC_GET_BLOCKING: picks the blocking flag when provided,
// defaults to false so non-blocking commands need no annotation.
#ifndef SYSTEC_GET_BLOCKING
#define SYSTEC_GET_BLOCKING(name, blocking, ...) blocking
#endif
...
/**
  * \brief SYSTEC plugin class definition.
  *
  * Wraps the uSystecCan SocketCAN driver (for SYS TEC electronic USB-CANmodul
  * devices, kernel module systec_can.ko) and exposes it through the standard
  ...
  *   FILTER  — installs SYSTEC hardware acceptance filters at runtime
  *   HWCTRL  — get/set SYS TEC device- and channel-specific sysfs controls
  *             (devicenr, reset, dual_channel, status_timeout,
  *             high_performance, channel, tx_timeout_ms) that have no
  *             SocketCAN-generic equivalent (see uSystecCan.hpp)
  *             without reopening the socket.
*/
class SYSTECPlugin: public PluginInterface
{
    public:
        ...
        */
        SYSTECPlugin() : m_strVersion(SYSTEC_PLUGIN_VERSION)
                    , m_strInstanceName(SYSTEC_PLUGIN_NAME)
        ...
        */
        bool setParams( const PluginDataSet *psSetParams )
        {
            ...
            if (true == generic_setparams<SYSTECPlugin>(this, psSetParams, &m_bIsFaultTolerant, &m_bIsPrivileged)) {
        ...
        */
        ...
        {
            generic_getparams<SYSTECPlugin>(this, psGetParams);
        }
        ...
        */
        ...
        {
            return generic_dispatch<SYSTECPlugin>(this, strCmd, strParams, st);
        }

        /**
          * \brief get a pointer to the plugin map
        */
        const PluginCommandsMap<SYSTECPlugin> *getMap(void) const
        {
            return &m_mapCmds;
        }
        ...
        */
        ...
        /**
          * \brief set the SYSTEC CAN ID stamped on outgoing frames, and mirror it onto
          ...
          *        opened socket (see m_SYSTEC_CMD / m_SYSTEC_SCRIPT) and the ones
        ...
        */
        bool setCanTpProtocol (const std::string& strProtocol) const
        {
            ...
            if (false == tp_protocol_from_string(strProtocol, eProto)) {
                LOG_PRINT(LOG_ERROR, LOG_STRING("SYSTEC |");
        ...
        // overridden here, via CONFIG (see systec_setup.hpp) or via INI
        ...
        */
        ...
        /**
          * \brief set SYSTEC write timeout
        */
        ...
        /**
          * \brief set SYSTEC read buffer size
          * \note Valid range is 1–8 bytes: systec_can.ko is classic-CAN only
          *       (see uSystecCan.hpp / CAN_DRV_MAX_DLEN), unlike the KVCAN
          *       plugin's 64-byte CAN FD ceiling.
        */
        bool setCanReadBufferSize (const std::string& strReadBufferSize) const
        {
            static constexpr uint32_t SYSTEC_CAN_MAX_DLEN = 8U;
            uint32_t u32Size = 0U;
            if (false == numeric::str2uint32(strReadBufferSize, u32Size)) {
                return false;
            }
            if (u32Size == 0U || u32Size > SYSTEC_CAN_MAX_DLEN) {
                LOG_PRINT(LOG_ERROR, LOG_STRING("SYSTEC |");
    ...
    private:
        ...
        /**
          * \brief message sender — matches PFSEND<SYSTECCAN> (see ICommDriver.hpp) so it can be
          *        handed directly to CommScriptCommandInterpreter/CommScriptClient as a pfsend
          *        override (see m_SYSTEC_CMD/m_SYSTEC_SCRIPT). Dispatches on m_eTpProtocol:
        ...
        */
        ...
        /**
          * \brief helper: parse a comma-separated filter list string into SYSTECCAN::CanFilter entries.
        ...
        */
        bool m_ParseFilters (const std::string& strFilters, std::vector<SYSTECCAN::CanFilter>& vFilters) const;

        /**
          * \brief map with association between the command string and the execution function
        */
        PluginCommandsMap<SYSTECPlugin> m_mapCmds;
        ...
        */
        ...
        /**
          ...
          *        (e.g. "SYSTEC" or "SYSTEC:1" -- see PluginDataSet::strInstanceName).
        ...
        */
        ...
        /**
          * \brief SYSTEC read timeout in milliseconds
        */
        mutable uint32_t m_u32ReadTimeout;

        /**
          * \brief SYSTEC write timeout in milliseconds
        */
        mutable uint32_t m_u32WriteTimeout;

        /**
          * \brief size of the buffer used for SYSTEC CAN read operations (max 8 bytes -- classic CAN only, see uSystecCan.hpp)
        */
        ...
        */
        mutable std::vector<SYSTECCAN::CanFilter> m_vFilters;

        /**
          * \brief functions associated to the plugin commands
        */
        #define SYSTEC_PLUGIN_CMD_RECORD(a, ...)  bool m_SYSTEC_##a ( const std::string& args, std::stop_token st ) const;
        SYSTEC_PLUGIN_COMMANDS_CONFIG_TABLE
        #undef  SYSTEC_PLUGIN_CMD_RECORD
};

#endif /* SYSTEC_PLUGIN_HPP */
