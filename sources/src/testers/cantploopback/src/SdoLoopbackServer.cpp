...
namespace {
    ...
    bool serve_download_from(const ICommDriver &bus, std::string_view rxId, std::string_view txId,
                             uint32_t timeoutMs, const Frame &req, std::vector<uint8_t> &outData)
    {
        outData.clear();
        if ((req[0] >> 5) != kCcsDlInitiate) {
            return false;
        }
        ...
        ack[1] = req[1];
        ack[2] = req[2];
        ack[3] = req[3]; // echo index/sub-index
        ...
        ack[0] = static_cast<uint8_t>((kScsDlInitiate << 5) | 1);
        if (!write_frame(bus, timeoutMs, txId, ack)) {
            return false;
        }

        while (outData.size() < totalLen) {
            Frame seg{};
            if (!read_frame(bus, timeoutMs, rxId, seg)) {
                return false;
            }
            ...
            if (!write_frame(bus, timeoutMs, txId, segAck)) {
                return false;
            }
    ...
    bool serve_upload_from(const ICommDriver &bus, std::string_view rxId, std::string_view txId,
                           uint32_t timeoutMs, const Frame &req, const std::vector<uint8_t> &data)
    {
        if ((req[0] >> 5) != kCcsUlInitiate) {
            return false;
        }
        ...
        resp[1] = req[1];
        resp[2] = req[2];
        resp[3] = req[3]; // echo index/sub-index
        ...
        if (!write_frame(bus, timeoutMs, txId, resp)) {
            return false;
        }
        ...
        while (sent < data.size()) {
            Frame segReq{};
            if (!read_frame(bus, timeoutMs, rxId, segReq)) {
                return false;
            }
            ...
            if (!write_frame(bus, timeoutMs, txId, segResp)) {
                return false;
            }
...
{
    Frame req{};
    if (!read_frame(bus, timeoutMs, rxId, req)) {
        return false;
    }
    return serve_download_from(bus, rxId, txId, timeoutMs, req, outData);
}
                                     ...
                                     uint32_t timeoutMs, const std::vector<uint8_t> &data)
{
    ...
    return serve_upload_from(bus, rxId, txId, timeoutMs, req, data);
}
                                  ...
                                  const std::function<void(bool isDownload)> &onDirectionKnown)
{
    ...
    if (ccs == kCcsDlInitiate) {
        ...
        return serve_download_from(bus, rxId, txId, timeoutMs, req, stored);
    }
    if (ccs == kCcsUlInitiate) {
        if (onDirectionKnown) {
            onDirectionKnown(false);
        }
        return serve_upload_from(bus, rxId, txId, timeoutMs, req, stored);
    }
