...
/**
 * @brief Legacy C-style function wrapper for port insertion
 * @deprecated Use uart::wait_for_insertion() instead
 */
[[deprecated("Use uart::wait_for_insertion() instead")]]
inline bool uart_wait_port_insert(
    char* port_buffer,
    uint32_t buffer_size,
    uint32_t timeout_ms,
    uint32_t polling_interval_ms) {
    
    if (!port_buffer || buffer_size == 0) {
        return false;
    }
    
    if (polling_interval_ms == 0 || 
        (timeout_ms != 0 && timeout_ms < polling_interval_ms)) {
        return false;
    }
    
    auto result = wait_for_insertion(timeout_ms, polling_interval_ms);
    
    if (result) {
        std::strncpy(port_buffer, result->c_str(), buffer_size - 1);
        port_buffer[buffer_size - 1] = '\0';
        return true;
    }
...
/**
 * @brief Legacy C-style function wrapper for port removal
 * @deprecated Use uart::wait_for_removal() instead
 */
[[deprecated("Use uart::wait_for_removal() instead")]]
inline bool uart_wait_port_remove(
    char* port_buffer,
    uint32_t buffer_size,
    uint32_t timeout_ms,
    uint32_t polling_interval_ms) {
    ...
    auto result = wait_for_removal(timeout_ms, polling_interval_ms);
...
/**
 * @brief Legacy get port count function
 * @deprecated Use uart::get_port_count() instead
 */
[[deprecated("Use uart::get_port_count() instead")]]
inline uint32_t uart_get_available_ports_number() {
    return static_cast<uint32_t>(get_port_count());
}

/**
 * @brief Legacy list ports function
 * @deprecated Use uart::list_ports() instead
 */
[[deprecated("Use uart::list_ports() instead")]]
inline void uart_list_ports(const char* = nullptr) {
    list_ports();
}
