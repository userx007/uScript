...
bool FT232HPlugin::m_handle_uart_help(const std::string &, std::stop_token /*st*/) const
{
...
bool FT232HPlugin::m_handle_uart_open(const std::string &strArgs, std::stop_token /*st*/) const
{
...
bool FT232HPlugin::m_handle_uart_close(const std::string &, std::stop_token /*st*/) const
{
    if (!m_pUART) {
...
bool FT232HPlugin::m_handle_uart_cfg(const std::string &strArgs, std::stop_token /*st*/) const
{
    if (strArgs == "help") {
        LOG_PRINT(LOG_EMPTY,
...
bool FT232HPlugin::m_handle_uart_write(const std::string &strArgs, std::stop_token st) const
{
    if (strArgs == "help") {
    ...
    auto r = pDrv->tout_write(pDrv->FT232H_UART_WRITE_DEFAULT_TIMEOUT, data, std::string_view{}, st);
...
bool FT232HPlugin::m_handle_uart_read(const std::string &strArgs, std::stop_token st) const
{
    if (strArgs == "help") {
    ...
    if (r.status != FT232HUART::Status::SUCCESS) {
        return false;
    }
...
bool FT232HPlugin::m_handle_uart_script(const std::string &strArgs, std::stop_token st) const
{
    if (strArgs == "help") {
